...
public class NativeAzureFileSystem extends FileSystem {
  ...
  /**
   * Checks if the given URI scheme is a scheme that's affiliated
  ...
  private static boolean isWasbScheme(String scheme) {
    ...
    return scheme != null &&
        (scheme.equalsIgnoreCase("asv") ||
            scheme.equalsIgnoreCase("asvs") ||
            scheme.equalsIgnoreCase("wasb") ||
            scheme.equalsIgnoreCase("wasbs"));
  }

  /**
   * Puts in the authority of the default file system if it is a WASB
   * file system and the given URI's authority is null.
   * @return The URI with reconstructed authority if necessary and possible.
   */
  private static URI reconstructAuthorityIfNeeded(URI uri,
      Configuration conf) {
  ...
  public FSDataOutputStream create(Path f, FsPermission permission,
    ...
    if (parentFolder != null &&
        parentFolder.getParent() != null) { // skip root
  ...
  boolean mkdirs(Path f, FsPermission permission, boolean noUmask) throws IOException {
    ...
    for (Path current = absolutePath, parent = current.getParent();
      ...
      if (currentMetadata != null && !currentMetadata.isDir()) {
        ...
        // The directory already exists. Its last modified time need to be updated
        // if there is a child directory created under it.
        if (childCreated) {
