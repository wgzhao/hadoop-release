 ...
 * send request events to Azure storage.
 */

//@@TODO rename
public class SendRequestIntercept extends StorageEvent<SendingRequestEvent>  {
   ...
   *
   * return allowConcurrentOOBIo - true if OOB I/O is allowed, and false otherwise.
   */
   ...
   * @param storageCreds - storage account credentials for signing packets.
   *
   */
  private SendRequestIntercept(StorageCredentials storageCreds, boolean allowConcurrentOOBIo,
   ...
   * can appropriately intercept sends. By allowing concurrent OOB I/Os, we bypass
   * the blob immutability check when reading streams.
   *
   * @param opContext The operation context to bind to listener.

   * @param allowConcurrentOOBIo True if reads are allowed with concurrent OOB writes.
   */
  public static void bind(StorageCredentials storageCreds, OperationContext opContext,
      boolean allowConcurrentOOBIo) {
    SendRequestIntercept sendListener =
        new SendRequestIntercept(storageCreds, allowConcurrentOOBIo, opContext);
   ...
   * The handler simply sets reset the conditional header to make all read
   * requests unconditional if reads with concurrent OOB writes are allowed.
   *
  ...
  public void eventOccurred(SendingRequestEvent sendEvent) {
    ...
    // Determine whether this is a download request by checking that the request method
    // is a "GET" operation.
    //
    if (urlConnection.getRequestMethod().equalsIgnoreCase("GET") && isOutOfBandIoAllowed()) {
      // If concurrent reads on OOB writes are allowed, reset the if-match condition on the
      // conditional header.
      //
      urlConnection.setRequestProperty(HeaderConstants.IF_MATCH, ALLOW_ALL_REQUEST_PRECONDITIONS);

      // In the Java AzureSDK the packet is signed before firing the SendRequest. Setting
      // the conditional packet header property changes the contents of the packet, therefore
      // the packet has to be re-signed.
      //
      try {
        // Sign the request. GET's have no payload so the content length is zero.
        //
        getCredentials().signRequest(urlConnection, -1L, getOperationContext());
      } catch (InvalidKeyException e) {
        // Log invalid key exception to track signing error before the send fails.
        //
