...
public class TestBlobMetadata {
  ...
  private static class FsWithPreExistingContainer implements Closeable {
    private final MockStorageInterface mockStorage;
    private final NativeAzureFileSystem fs;

    private FsWithPreExistingContainer(
  ...
  /**
   * Tests that WASB stamped the version in the container metadata if
  ...
  public void testPreExistingContainerVersionMetadata() throws Exception {
    ...
    FsWithPreExistingContainer fsWithContainer =
        FsWithPreExistingContainer.create();
    ...
    assertEquals(AzureNativeFileSystemStore.CURRENT_WASB_VERSION,
        fsWithContainer.getContainerMetadata().
          get(AzureNativeFileSystemStore.VERSION_METADATA_KEY));
    fsWithContainer.close();
  }

  /**
   * Tests that WASB works well with an older version container
   * with ASV-era version and metadata.
   */
  @Test
  public void testFirstContainerVersionMetadata() throws Exception {
    ...
    FsWithPreExistingContainer fsWithContainer =
        FsWithPreExistingContainer.create(containerMetadata);
    ...
    assertEquals(
        AzureNativeFileSystemStore.FIRST_WASB_VERSION,
        fsWithContainer.getContainerMetadata().
          get(AzureNativeFileSystemStore.OLD_VERSION_METADATA_KEY));
    assertNull(fsWithContainer.getContainerMetadata().
        get(AzureNativeFileSystemStore.VERSION_METADATA_KEY));
  ...
  /**
   * Tests that WASB understands the old-style ASV metadata and
   * changes it when it gets the chance.
   */
  @Test
