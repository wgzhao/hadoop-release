...
public class TestRecoverStripedFile {
  ...
  private int generateErrors(Map<ExtendedBlock, Integer> corruptTargets,
      RecoveryType type)
    throws IOException {
    int stoppedDN = 0;
    for (Map.Entry<ExtendedBlock, Integer> target : corruptTargets.entrySet()) {
      if (stoppedDN == 0 || type != RecoveryType.DataOnly
          || random.nextBoolean()) {
        // stop at least one DN to trigger recovery
        LOG.info("Note: stop DataNode " + target.getValue()
            + " with internal block " + target.getKey());
        shutdownDataNodes(cluster.getDataNodes().get(target.getValue()));
        stoppedDN++;
      } else { // corrupt the data on the DN
        LOG.info("Note: corrupt data on " + target.getValue()
            + " with internal block " + target.getKey());
  ...
  private void assertFileBlocksRecovery(String fileName, int fileLen,
    ...
    Map<ExtendedBlock, Integer> errorMap = new HashMap<>(dead.length);
    for (int i = 0; i < toRecoverBlockNum; i++) {
      ...
      errorMap.put(blocks[i], deadDnIndices[i]);
