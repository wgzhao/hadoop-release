...
public class TestReconstructStripedBlocksWithRackAwareness {
  ...
  public void testReconstructForNotEnoughRacks() throws Exception {
    ...
    for (DatanodeStorageInfo storage : bm.getStorages(blockInfo)) {
    ...
    cluster.corruptBlockOnDataNodesByDeletingBlockFile(
        new ExtendedBlock(fsn.getBlockPoolId(), internalBlock));
    ...
    badDn.reportBadBlocks(new ExtendedBlock(fsn.getBlockPoolId(), internalBlock));
