...
public class FSNamesystem implements Namesystem, FSClusterStats,
    ...
    for (int attempt = 0; attempt < 2; attempt++) {
      ...
      try {
        ...
        final LocatedBlocks blocks =
          blockManager.createLocatedBlocks(inode.getBlocks(iip.getPathSnapshotId()), fileSize,
    ...
    if(newBlock == null) {
      newBlock = (shouldCopyOnTruncate) ? createNewBlock() :
          new Block(oldBlock.getBlockId(), oldBlock.getNumBytes(),
              nextGenerationStamp(isLegacyBlock(oldBlock)));
    }
      ...
      if (myFile == null) {
        ...
        if (overwrite) {
          ...
          recoverLeaseInternal(RecoverLeaseOp.CREATE_FILE,
              myFile, src, holder, clientMachine, false);
  ...
  boolean recoverLease(String src, String holder, String clientMachine)
    ...
    try {
      ...
      return recoverLeaseInternal(RecoverLeaseOp.RECOVER_LEASE,
          inode, src, holder, clientMachine, true);
    } catch (StandbyException se) {
      skipSync = true;
      throw se;
    } finally {
  ...
  boolean recoverLeaseInternal(RecoverLeaseOp op, INodeFile fileInode,
  ...
  private static class DefaultAuditLogger extends HdfsAuditLogger {
    ...
    public void initialize(Configuration conf) {
      isCallerContextEnabled = conf.getBoolean(
          DFSConfigKeys.DFS_CALLER_CONTEXT_ENABLED_KEY,
          DFSConfigKeys.DFS_CALLER_CONTEXT_ENABLED_DEFAULT);
      callerContextMaxLen = conf.getInt(
          DFSConfigKeys.DFS_CALLER_CONTEXT_MAX_SIZE_KEY,
          DFSConfigKeys.DFS_CALLER_CONTEXT_MAX_SIZE_DEFAULT);
      callerSignatureMaxLen = conf.getInt(
          DFSConfigKeys.DFS_CALLER_CONTEXT_SIGNATURE_MAX_SIZE_KEY,
          DFSConfigKeys.DFS_CALLER_CONTEXT_SIGNATURE_MAX_SIZE_DEFAULT);
      ...
      if (auditLog.isInfoEnabled()) {
        ...
        if (isCallerContextEnabled &&
          ...
          if (callerContext.getSignature() != null) {
            sb.append(":");
            if (callerContext.getSignature().length() > callerSignatureMaxLen) {
              sb.append(callerContext.getSignature().substring(0,
                  callerSignatureMaxLen));
            } else {
              sb.append(callerContext.getSignature());
            }
          }
        }
