...
public class BlockManager implements BlockStatsMXBean {
  ...
  private void addToInvalidates(Block b) {
    ...
    if (datanodes.length() != 0) {
      blockLog.debug("BLOCK* addToInvalidates: {} {}", b, datanodes.toString());
    }
  }
  ...
  private static class ReplicationStats {
    private int blocksWithoutTargets = 0;
    private int blocksWithoutSource = 0;
    private int blocksAbandoned = 0;
    private int blocksUnderConstruction = 0;
    private int blocksWithEnoughReplicas = 0;
      ...
      synchronized (neededReplications) {
        for (int priority = 0; priority < blocksToReplicate.size(); priority++) {
          for (BlockInfo block : blocksToReplicate.get(priority)) {
            ReplicationWork rw = scheduleReplication(block, priority, stats);
            if (rw != null) {
              work.add(rw);
            }
          }
    ...
    try {
      for(ReplicationWork rw : work){
        ...
        if(targets == null || targets.length == 0){
          rw.resetTargets();
          stats.blocksWithoutTargets++;
          continue;
        }

        synchronized (neededReplications) {
          if (validateReplicationWork(rw, stats)) {
            scheduledWork++;
          }
        }
      }
  ...
  private ReplicationWork scheduleReplication(BlockInfo block, int priority,
    ...
    if (bc == null
      ...
      if (bc == null) {
        stats.blocksAbandoned++;
      } else {
        stats.blocksUnderConstruction++;
      }
      return null;
    }
