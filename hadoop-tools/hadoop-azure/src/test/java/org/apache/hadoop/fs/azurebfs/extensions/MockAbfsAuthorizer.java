...
public class MockAbfsAuthorizer implements AbfsAuthorizer {
  ...
  public boolean isAuthorized(FsAction action, Path... absolutePaths) {
    ...
    if (action.equals(FsAction.READ) && isAllElementInOtherSets(paths, readOnlyPaths, readWritePaths)) {
      return true;
    } else if (action.equals(FsAction.READ) && paths.contains(new Path(TEST_WRITE_THEN_READ_ONLY)) && writeThenReadOnly == 1) {
      return true;
    } else if (action.equals(FsAction.WRITE) && isAllElementInOtherSets(paths, writeOnlyPaths, readWritePaths)) {
      return true;
  ...
  @SafeVarargs
  private final boolean isAllElementInOtherSets(Set<Path> paths, Set<Path>... sets) {
    Set<Path> concatanetedSets = new HashSet<>();
    for (Set<Path> set : sets) {
      concatanetedSets.addAll(set);
    }
    return concatanetedSets.containsAll(paths);
  }
}
