...
public class LocalSASKeyGeneratorImpl extends SASKeyGeneratorImpl {
   ...
   */
  Map<String, CloudStorageAccount> storageAccountMap;
  ...
  public URI getContainerSASUri(String accountName, String container)
    ...
    try {
      ...
      throw new SASKeyGenerationException("Encountered StorageException while generating SAS Key for container " +
          container + " inside storage account " + accountName, stoEx);
    } catch (URISyntaxException uriSyntaxEx) {
      throw new SASKeyGenerationException("Encountered URISyntaxException while generating SAS Key for container " +
          container + " inside storage account " + accountName, uriSyntaxEx);
    }
  }
   ...
   * @return CloudStorageAccount instance created using SAS key for the Storage Account.
   * @throws SASKeyGenerationException
   */
  private CloudStorageAccount getSASKeyBasedStorageAccountInstance(String accountName)
      throws SASKeyGenerationException {
    ...
    try {
      ...
      CloudStorageAccount account = getStorageAccountInstance(accountNameWithoutDomain,
          AzureNativeFileSystemStore.getAccountKeyFromConfiguration(accountName, conf));

      return new CloudStorageAccount(new StorageCredentialsSharedAccessSignature(
          account.generateSharedAccessSignature(getDefaultAccountAccessPolicy())), false,
          account.getEndpointSuffix(), accountNameWithoutDomain);

    } catch (KeyProviderException keyProviderEx) {
      throw new SASKeyGenerationException("Encountered KeyProviderException while retrieving Storage"
          + " key from configuration for account " + accountName, keyProviderEx);
    } catch (InvalidKeyException invalidKeyEx) {
      throw new SASKeyGenerationException( "Encoutered InvalidKeyException while generating Account"
          + " level SAS key for account" + accountName, invalidKeyEx);
    } catch(StorageException storeEx) {
      throw new SASKeyGenerationException("Encoutered StorageException while generating Account"
          + " level SAS key for account" + accountName, storeEx);
    } catch(URISyntaxException uriSyntaxEx) {
      throw new SASKeyGenerationException("Encountered URISyntaxException for account "
          + accountName, uriSyntaxEx);
    }
  }
  ...
  public URI getRelativeBlobSASUri(String accountName, String container,
    ...
    try {
      ...
      throw new SASKeyGenerationException("Encountered StorageException while getting "
          + "container references for container " + container +
    ...
    try {
      ...
      throw new SASKeyGenerationException("Encountered URISyntaxException while getting"
          + " Block Blob references for container " + container
          + " inside storage account : " + accountName, uriSyntaxEx);
    } catch (StorageException stoEx) {
      throw new SASKeyGenerationException("Encountered StorageException while getting "
          + "Block Blob references for container " + container
    ...
    try {
      ...
      throw new SASKeyGenerationException("Encountered StorageException while generating "
          + "SAS key for Blob: " + relativePath + " inside container : " + container
          + " in Storage Account : " + accountName, stoEx);
    } catch (URISyntaxException uriSyntaxEx) {
      throw new SASKeyGenerationException("Encountered URISyntaxException while generating "
          + "SAS key for Blob: " + relativePath + " inside container : " + container
          + " in Storage Account : " + accountName, uriSyntaxEx);
    }
  }

  /**
   * Helper method that creates CloudStorageAccount Instance using the storage account
   * key.
   ...
   * @throws SASKeyGenerationException
   */
  private CloudStorageAccount getStorageAccountInstance(String accountName, String accountKey)
      throws SASKeyGenerationException {
    ...
    if (!storageAccountMap.containsKey(accountName)) {
      ...
      try {
        account =
            new CloudStorageAccount(new StorageCredentialsAccountAndKey(accountName,
                  accountKey));
      } catch (URISyntaxException uriSyntaxEx) {
  ...
  private SharedAccessAccountPolicy getDefaultAccountAccessPolicy() {
    ...
    cal.add(Calendar.HOUR, sasKeyExpiryPeriod * 24);
  ...
  private EnumSet<SharedAccessAccountPermissions> getDefaultAccoutSASKeyPermissions() {
    return EnumSet.of(SharedAccessAccountPermissions.ADD, SharedAccessAccountPermissions.CREATE,
        SharedAccessAccountPermissions.DELETE, SharedAccessAccountPermissions.LIST, SharedAccessAccountPermissions.READ,
