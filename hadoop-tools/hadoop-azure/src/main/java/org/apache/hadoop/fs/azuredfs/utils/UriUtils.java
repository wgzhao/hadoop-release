...
public final class UriUtils {
  ...
  /**
   * Checks whether a string includes adfs url.
   * @param string the string to check.
   * @return true if string has adfs url.
   */
  public static boolean containsAdfsUrl(final String string) {
    if (string == null || string.isEmpty()) {
      return false;
    }

    return ADFS_URI_PATTERN.matcher(string).matches();
  }
  ...
  public static String extractRawAccountFromAccountName(final String accountName) {
    ...
    if (!containsAdfsUrl(accountName)) {
      return null;
    }
