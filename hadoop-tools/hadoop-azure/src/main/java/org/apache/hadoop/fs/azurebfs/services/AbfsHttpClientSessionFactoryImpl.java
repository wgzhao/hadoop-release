...
final class AbfsHttpClientSessionFactoryImpl implements AbfsHttpClientSessionFactory {
  private static final String AZURE_DISTRIBUTED_FILE_SYSTEM_AUTHORITY_DELIMITER = "@";
  private final ConfigurationService configurationService;
  private final LoggingService loggingService;

  @Inject
  AbfsHttpClientSessionFactoryImpl(
      final ConfigurationService configurationService,
      final LoggingService loggingService) {
    Preconditions.checkNotNull(configurationService, "configurationService");
    Preconditions.checkNotNull(loggingService, "loggingService");
    this.configurationService = configurationService;
    this.loggingService = loggingService.get(AbfsHttpClientSessionFactory.class);
  }

  @Override
  public AbfsHttpClientSession create(final FileSystem fs) throws AzureBlobFileSystemException {
    this.loggingService.debug(
        "Creating AbfsHttpClientSession for filesystem: {0}", fs.getUri());
