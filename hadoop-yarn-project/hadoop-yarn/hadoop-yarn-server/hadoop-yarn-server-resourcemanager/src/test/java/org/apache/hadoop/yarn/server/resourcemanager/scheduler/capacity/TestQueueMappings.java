...
public class TestQueueMappings {
  ...
  private MockRM resourceManager1;
  private MockRM resourceManager2;

  @After
  public void tearDown() throws Exception {
    if (resourceManager1 != null) {
        LOG.info("Stopping the resource manager");
        resourceManager1.stop();
      }
    if (resourceManager2 != null) {
      LOG.info("Stopping the resource manager");
      resourceManager2.stop();
    }
  }
  ...
  public void testQueueMapping() throws Exception {
    ...
    resourceManager1 = new MockRM(conf);
    ...
    conf.set(CapacitySchedulerConfiguration.ENABLE_QUEUE_MAPPING_OVERRIDE,
        "true");
    CapacityScheduler cs = (CapacityScheduler) resourceManager1.getResourceScheduler();
    ...
    resourceManager2 = new MockRM(csConf);
    ...
    resourceManager2.getResourceScheduler().reinitialize(conf, null);

    // ensure that if the user specifies a Q that is still overriden
    checkAppQueue(resourceManager2, "user", Q2, Q1);
    ...
    checkAppQueue(resourceManager2, "user", Q2, Q2);

    // ensure that if a user does not specify a Q, the user mapping is used
    checkAppQueue(resourceManager2, "user", null, Q1);
    ...
    checkAppQueue(resourceManager2, "user", null, Q2);
