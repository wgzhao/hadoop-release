...
public class SchedulerUtils {
  ...
  public static void validateResourceRequest(ResourceRequest resReq,
    ...
    String labelExp = resReq.getLabelExpression();
    
    // if queue has default label expression, and RR doesn't have, use the
    // default label expression of queue
    if (labelExp == null && queueInfo != null) {
      labelExp = queueInfo.getDefaultLabelExpression();
      resReq.setLabelExpression(labelExp);
    }
    
    if (labelExp != null && !labelExp.trim().isEmpty() && queueInfo != null) {
      if (!checkQueueLabelExpression(queueInfo.getLabels(),
          labelExp)) {
        throw new InvalidResourceRequestException("Invalid resource request"
            + ", queue="
  ...
  public static boolean checkQueueAccessToNode(Set<String> queueLabels,
    ...
    if (queueLabels != null && queueLabels.contains(NodeLabelManager.ANY)) {
      return true;
    }
  ...
  public static void checkAndThrowIfLabelNotIncluded(NodeLabelManager mgr,
