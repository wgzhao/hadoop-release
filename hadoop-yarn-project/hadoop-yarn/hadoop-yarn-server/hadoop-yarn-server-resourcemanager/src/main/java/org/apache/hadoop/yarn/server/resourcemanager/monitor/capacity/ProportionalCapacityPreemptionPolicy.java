...
public class ProportionalCapacityPreemptionPolicy implements SchedulingEditPolicy {
  ...
  private void resetCapacity(ResourceCalculator rc, Resource clusterResource,
    ...
    int availableQueueSize = 0;
    
    // check if it's possible to get any available resource for this queue
    for (TempQueue q : queues) {
      if (q.children.isEmpty()
          && !hasAvailableResourceAccordingToLabel(q, clusterResource)) {
        // a queue's normalized guarantee set -infinity means it cannot get any
        // resource now, will ignore compute it's normalizedGuarantee 
        q.normalizedGuarantee = Double.NEGATIVE_INFINITY;
      } else {
        availableQueueSize++;
      }
    }
