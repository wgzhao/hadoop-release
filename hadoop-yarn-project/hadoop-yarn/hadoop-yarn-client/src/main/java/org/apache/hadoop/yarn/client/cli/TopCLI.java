...
public class TopCLI extends YarnCLI {
  ...
  private static class ApplicationInformation {
    ...
    ApplicationInformation(ApplicationReport appReport) {
      displayStringsMap = new EnumMap<Columns, String>(Columns.class);
  ...
  public TopCLI() throws IOException, InterruptedException {
    ...
    currentSortField = "c";
    keyFieldsMap = new HashMap<String, Columns>();
    runKeyboardMonitor = new AtomicBoolean();
    runMainLoop = new AtomicBoolean();
    runKeyboardMonitor.set(true);
    runMainLoop.set(true);
    displayThread = Thread.currentThread();
    columnInformationEnumMap = new EnumMap<Columns, ColumnInformation>(Columns.class);
    generateColumnInformationMap();
    generateKeyFieldsMap();
    sortedKeys = new ArrayList<String>(keyFieldsMap.keySet());
  ...
  private void setAppsHeader() {
    ...
    if (appsHeader.length() > terminalWidth) {
      ...
      appsHeader +=
          StringUtils.repeat(" ", terminalWidth - appsHeader.length() - System
              .getProperty("line.separator").length());
    }
    appsHeader += System.getProperty("line.separator");
  }
  ...
  protected QueueMetrics getQueueMetrics() {
    ...
    if (queues.isEmpty()) {
      ...
      queuesInfo = new ArrayList<QueueInfo>();
    ...
    for (int i = 0; i < limit; ++i) {
      ...
      if(i < appsInfo.size()) {
        ...
        ret.append(limitLineLength(
            (StringUtils.join(columns.toArray(), " ") + System
                .getProperty("line.separator")), terminalWidth, true));
      }
      else {
  ...
  protected void showTopScreen() {
    List<ApplicationInformation> appsInfo = new ArrayList<ApplicationInformation>();
    ...
    if (line.length() > length) {
      ...
      if (addNewline) {
        tmp = line.substring(0,
            length - System.getProperty("line.separator").length());
