...
public class TimelineClientImpl extends TimelineClient {
  ...
  public TimelinePutResponse putEntities(CacheId cacheId,
      ...
      throws IOException, YarnException {
    if (!timelineServerPluginEnabled || cacheId == null
        || appAttemptId == null) {
    ...
    for (TimelineEntity entity : entities) {
      ...
      if (summaryEntityTypes.contains(entity.getEntityType())) {
        entityLogPath =
            new Path(createAttemptDir(cacheId, appAttemptId),
              SUMMARY_LOG_PREFIX + appAttemptId.toString());
        LOG.info("Writing summary log for " + appAttemptId.toString() + " to "
            + entityLogPath);
      } else {
        entityLogPath =
            new Path(createCacheDir(cacheId, appAttemptId), ENTITY_LOG_PREFIX
                + cacheId.toString());
        LOG.info("Writing entity log for " + cacheId.toString() + " to "
            + entityLogPath);
      }
      writeEntity(entity, entityLogPath);
    }
  ...
  @Override
  public void putDomain(CacheId cacheId, ApplicationAttemptId appAttemptId,
      TimelineDomain domain) throws IOException, YarnException {
    if (!timelineServerPluginEnabled || cacheId == null
        || appAttemptId == null) {
      putDomain(domain);
    } else {
      writeDomain(cacheId, appAttemptId, domain);
    }
  }
  ...
  private Path createAttemptDir(CacheId cacheId,
      ApplicationAttemptId appAttemptId) throws IOException {
    Path appDir = new Path(activePath, cacheId.getApplicationId().toString());
    if (!fs.exists(appDir)) {
  ...
  private Path createCacheDir(CacheId cacheId,
      ApplicationAttemptId appAttemptId) throws IOException {
    Path attemptDir = createAttemptDir(cacheId, appAttemptId);

    Path cacheDir = new Path(attemptDir, cacheId.toString());
    if (!fs.exists(cacheDir)) {
      FileSystem
        .mkdirs(fs, cacheDir, new FsPermission(APP_LOG_DIR_PERMISSIONS));
    }
    return cacheDir;
  }

  private void writeDomain(CacheId cacheId, ApplicationAttemptId appAttemptId,
      TimelineDomain domain) throws IOException {
    Path domainLogPath =
        new Path(createAttemptDir(cacheId, appAttemptId), DOMAIN_LOG_PREFIX
