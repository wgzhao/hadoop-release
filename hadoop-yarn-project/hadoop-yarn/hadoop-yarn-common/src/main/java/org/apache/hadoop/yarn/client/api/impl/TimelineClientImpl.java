...
public class TimelineClientImpl extends TimelineClient {
  ...
  public TimelinePutResponse putEntities(CacheId cacheId,
    ...
    for (TimelineEntity entity : entities) {
      Path cacheDir = createCacheDir(cacheId);
      Path entityLogPath;
  ...
  @Override
  public void putDomain(CacheId cacheId, TimelineDomain domain)
      throws IOException, YarnException {
    if (!timelineServerPluginEnabled || cacheId == null) {
      putDomain(domain);
    } else {
      writeDomain(cacheId, domain);
    }
  }
  ...
  private Path createCacheDir(CacheId cacheId) throws IOException {
    Path appDir =
        new Path(activePath, cacheId.getApplicationAttemptId()
          .getApplicationId().toString());
    if (!fs.exists(appDir)) {
      FileSystem.mkdirs(fs, appDir, new FsPermission(APP_LOG_DIR_PERMISSIONS));
    }
    Path cacheDir = new Path(appDir, cacheId.toString());
  ...
  private void writeDomain(CacheId cacheId,
      TimelineDomain domain) throws IOException {
    Path domainLogPath =
        new Path(createCacheDir(cacheId), DOMAIN_LOG_PREFIX
            + cacheId.toString());
